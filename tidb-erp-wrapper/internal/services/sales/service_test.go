...
func TestSalesService(t *testing.T) {
	...
	t.Run("CreateCustomer", func(t *testing.T) {
		...
		err = svc.CreateCustomer(ctx, dupCustomer)
		assert.Error(t, err)
	})
	...
	t.Run("CreatePriceList", func(t *testing.T) {
		...
		err := svc.CreatePriceList(ctx, priceList, items)
		assert.NoError(t, err)
		...
		err = svc.CreatePriceList(ctx, emptyPriceList, nil)
		assert.Error(t, err)
		...
		err = svc.CreatePriceList(ctx, priceList, invalidItems)
		assert.Error(t, err)
	})
	...
	t.Run("CreateSalesOrder", func(t *testing.T) {
		...
		err = svc.CreatePriceList(ctx, priceList, priceListItems)
		require.NoError(t, err)
		...
		err = svc.CreateSalesOrder(ctx, order, items)
		assert.NoError(t, err)
		...
		err = svc.CreateSalesOrder(ctx, exceedingOrder, items)
		assert.Error(t, err)
		...
		err = svc.CreateSalesOrder(ctx, invalidOrder, items)
		assert.Error(t, err)
		...
		err = svc.CreateSalesOrder(ctx, mismatchOrder, items)
		assert.Error(t, err)
	...
	t.Run("CreateDelivery", func(t *testing.T) {
		...
		err = svc.CreateSalesOrder(ctx, order, orderItems)
		require.NoError(t, err)
		...
		err = svc.CreateDelivery(ctx, delivery, items)
		assert.NoError(t, err)
		...
		err = svc.CreateSalesOrder(ctx, unconfirmedOrder, orderItems)
		require.NoError(t, err)
		...
		err = svc.CreateDelivery(ctx, invalidDelivery, items)
		assert.Error(t, err)
		...
		err = svc.CreateDelivery(ctx, excessDelivery, excessItems)
		assert.Error(t, err)
	...
	t.Run("TransactionRollback", func(t *testing.T) {
		...
		err = svc.CreateSalesOrder(ctx, order, invalidItems)
		assert.Error(t, err)

		// Verify the order was not created
		_, _, err = svc.GetSalesOrderByID(ctx, order.ID)
		assert.Error(t, err)
	})
	...
	t.Run("ConcurrentOrders", func(t *testing.T) {
		...
		rows, err := testDB.DB.DB().QueryContext(ctx,
